...
class CompareSheetReader:
    ...
    def IsHasTitleDiff(self,target_sheet_reader,base_sheet_reader):
        ...
        for merge_cell in base_sheet_reader.sheet.merged_cells.ranges:
            if base_sheet_reader._isInCboundsMergeCell(merge_cell,target_bounds):
        ...
        base_vluDic = self._getStrValuesDic(target_sheet_reader.sheet,base_bounds)
    ...
    def HasSameCboundDiff(self,reader1,reader2,interse_set):
        for name in interse_set:
            ...
            if mine_key_diffs and their_key_diffs:
                ...
                if len(diff_set) > 0:
                    return True,name,diff_set
        return False,"",diff_set
    ...
    def _collectKeysDiffType(self,target_sheet_reader,base_sheet_reader):
        ...
        for key in interse_set:
            if self.HasBoundsDiff(key,target_sheet_reader,base_sheet_reader):
        ...
        if len(result["del_key"]) > 0:
            ...
            for item_key in cache:
                DebugHelper.LogColor(DebugHelper.FontColor.pink,"【删除】{} {} {}".format(target_sheet_reader.excel_title,target_sheet_reader.sheet.title,item_key))
